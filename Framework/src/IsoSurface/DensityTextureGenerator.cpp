...
namespace Engine
{
	...
	void DensityTextureGenerator::PerlinFBM(const PerlinNoiseSettings& args)
	{
	...
	void DensityTextureGenerator::Smooth(const CSGOperationSettings& settings)
	{
		...
		ComputeContext->CommandList->SetComputeRoot32BitConstants(0, 1, &settings.DensityType, 1);
		ComputeContext->CommandList->SetComputeRoot32BitConstants(0, 1, &settings.Operation, 2);
	...
	void DensityTextureGenerator::BuildComputeRootSignature()
	{
		...
		slotRootParameter[1].InitAsConstants(2, 1);					// csg settings
